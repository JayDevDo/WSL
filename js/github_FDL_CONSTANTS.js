...
let gamesOverview = {
		...
		finishedRounds: 14,
		currentRnd: 15,
		evWndw: { 'direction': 1 , 'start': 15, 'rounds': 5, 'end': 19 },
...
hasUserStore = ()=>{
	let hasLS = false ;
	try{
		if( JSON.parse( localStorage.wslusrdf ) ){ hasLS = true; }else{ hasLS = false; }
...
setUserDF = ()=>{
	...
	if( localStorage.wslusrdf ){ delUserDF() }
	...
	localStorage.wslusrdf = JSON.stringify( lclStrgArr ) ;
...
delUserDF = ()=>{
	...
	localStorage.wslusrdf = JSON.stringify( [] );
}
...
loadUserDF = ()=>{
	...
	if( localStorage.wslusrdf ){

		// console.log( "loadUserDF: localStorage exists. =", JSON.parse( localStorage.usrdf ) ) ;
		// console.log( "loadUserDF: localStorage exists. =", JSON.parse( localStorage.usrdf ) ) ;
		storedH = JSON.parse( localStorage.wslusrdf ) ;

		if( storedH.length != 12 ){ setUserDF() ; }
...
tmFilterReset = ()=>{
	gamesOverview.teamFilter =[ true, true , true , true , true , true , true , true , true , true , true , true , true] ;
	$( "#slctdTeams" ).val( "a" ) ;
