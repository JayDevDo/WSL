...
handlePostponed = (fxtr, loc)=>{
	/* remove fxtr if already added to team row 23 */
	/* adds div elements to the td in the fxtrTbl table column round 23 */
	let fxtrppExists, pptarget_td, pptarget_div, pptarget_txt, pptarget_count

	fxtrppExists = $("#fxtrTbl span.fxtrspan[evrnd='23'][loc=' "+ loc +"'][fxtrid=" + fxtr.id + "]").remove();

	let pptarget_arr = ["<span", 
						" teamId_h=", fxtr.team_h,
						" teamId_a=", fxtr.team_a,
						" fxtrid=", fxtr.id,
						" evrnd=", fxtr.event /* should always be 23 */,
						" ogevrnd=", fxtr.ogGW, 
						" nwevrnd=", fxtr.event,
